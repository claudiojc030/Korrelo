...
export class DeployRecord {
  ...
  appendLog(chunk: string): DeployRecord {
    ...
    return new DeployRecord(this.id, this.projectId, this.status, this.triggeredBy, this.errorMessage, log, this.startedAt, this.finishedAt);
  }

  withResult(status: "success" | "failed", errorMessage: string | null): DeployRecord {
    return new DeployRecord(this.id, this.projectId, status, this.triggeredBy, errorMessage, this.log, this.startedAt, new Date());
  }
}
