...
function buildUseCase(overrides?: {
  ...
  const user = overrides?.user !== undefined ? overrides.user : User.create("admin", "hashed");
...
describe("LoginUseCase", () => {
  it("retorna um access token quando as credenciais são válidas", async () => {
    ...
    const result = await useCase.execute({ username: "admin", password: "correta" });
    ...
    expect(tokenPairIssuer.issue).toHaveBeenCalledWith(expect.any(String), "admin", null, null);
  });
  ...
  it("rejeita quando o usuário não existe", async () => {
    ...
    await expect(
      useCase.execute({ username: "inexistente", password: "qualquer" }),
  ...
  it("rejeita quando a senha está incorreta", async () => {
    ...
    await expect(
      useCase.execute({ username: "admin", password: "errada" }),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });

  it("pede o segundo fator quando 2FA está ativado e nenhum código foi enviado", async () => {
    const userWith2fa = User.create("admin", "hashed").withTwoFactorEnabled(["hash1"]);
  ...
  it("aceita login com 2FA quando o código TOTP é válido", async () => {
    ...
    const result = await useCase.execute({ username: "admin", password: "correta", twoFactorCode: "123456" });
  ...
  it("rejeita login com 2FA quando o código é inválido e não bate com nenhum backup code", async () => {
    ...
    await expect(
      useCase.execute({ username: "admin", password: "correta", twoFactorCode: "000000" }),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });
});
